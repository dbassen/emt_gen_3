...
linelist = lin.reader('DataFile.jl')
...
for x,i in zip(linelist,range(len(linelist))):
    ...
    if 'SHEAR_STRESS' in x and 'initial_condition_array' in x and '_2' not in x:
        SHEAR_STRESS_i =pull_index_df_rIca(x) 
    if 'SHEAR_STRESS_2' in x and 'initial_condition_array' in x:
        SHEAR_STRESS_2_i =pull_index_df_rIca(x)
...
for line in bmods:
    ...
    if 'SHEAR_STRESS' in line:
        app = line.replace('SHEAR_STRESS',str(SHEAR_STRESS_i))
    if 'SHEAR_STRESS_2' in line:
        app = line.replace('SHEAR_STRESS_2',str(SHEAR_STRESS_i))
