...
# addition indices
addition_start,addition_end = 105,124

# mRNA / transcriptional indices
mdeg_start,mdeg_end = 125,152
script_start,script_end = 153,180

# protein / translational indices
pdeg_start,pdeg_end = 45,83
lation_start,lation_end = 181,208
...
@debug function SolveModelGeneric_ram(TSTART,TSTOP,Ts,data_dictionary_active,sim_RCA,sim_ICA)
  include("../src/edit_file_global.jl")
  ...
  if sim_ICA != []
  UpdateArray(data_dictionary_active,"INITIAL_CONDITION_ARRAY",sim_ICA)
  #println(sim_ICA)
  end
  # @show sim_RCA
...
function SolveModelGenericPOETS_ram(TSTART,TSTOP,Ts,local_data_dictionary,sim_RCA,sim_ICA)
 ...
 # Just try to adjust dosage level by similiar amount as protein (?)
  ...
  if sim_ICA != []#WCB
  UpdateArray(local_data_dictionary,"INITIAL_CONDITION_ARRAY",sim_ICA)
  #println(sim_ICA)
  end
# factor of 10 just corrects for a 0.1 "gamma/delta" value in the data file
  rate_constant_array[mdeg_start:mdeg_end]=10*rate_constant_array[mdeg_start:mdeg_end]*degradation_constant_mRNA
...
function SolveModelGeneric_ram_regen(TSTART,TSTOP,Ts,data_dictionary_active,sim_RCA,sim_ICA)
