...
##### HARDCODED##
BIG = 1e10
num_objs = 8 # changes for cross validation
SMALL = 1e-3
...
fpath="../poets/shear_data_combined.jl" #WCB
include(fpath)
...
#     #    #   # Initialize and  Run model to steady state
 ...
 # # # #
##### HARDCODED##
 # update control parameter array here for now #wcb
...
# ROCK_inhibition_Sequester_YAP1: ROCK inhibition; Sequester_YAP1
control_parameter_array[6,1] = 0.5;	#	6 Gain: 	ROCK_inhibition_Sequester_YAP1 target: Sequester_YAP1 actor: ROCK type: inhibition
control_parameter_array[6,2] = 1.0;	#	6 Order: 	ROCK_inhibition_Sequester_YAP1 target: Sequester_YAP1 actor: ROCK type: inhibition

# Akt_acitvation_Sequester_YAP1: pAkt induction; Sequester_YAP1
control_parameter_array[7,1] = 5;	#	7 Gain: 	Akt_acitvation_Sequester_YAP1 target: Sequester_YAP1 actor: pAkt type: induction
...
control_parameter_array[8,1]=0.3 # GAIN # activation_Ecadherin_by_Active_NFATc
# repression_Ecadherin_by_pSmad: pSmad repression induction_gene_Ecadherin
control_parameter_array[9,1]=5.0 # GAIN # REP_ECAD_PSMAD
control_parameter_array[9,2]=2.0 # ORDER # REP_ECAD_PSMAD
# repression_Ecadherin_by_SNAIL_SLUG: SNAIL_SLUG repression induction_gene_Ecadherin
control_parameter_array[10,1]=0.5 # GAIN_SNAIL_SLUG_ECAD #2.0
# repression_Ecadherin_by_Active_LEF1: Active_LEF1 repression induction_gene_Ecadherin
control_parameter_array[11,1]=0.6# GAIN_ACTIVE_LEF1_ECAD #2.0 0.5
control_parameter_array[11,2]=1.0# ORDER_ACTIVE_LEF1_ECAD
...
control_parameter_array[12,1]=5.0 # ecad_virus ***
# activation_TGFB3_by_TCF4: Active_TCF4 induction induction_gene_TGFB3
control_parameter_array[13,1]=0.15 # GAIN_TCF4_TGFB3
control_parameter_array[13,2]=1.5 # GAIN_TCF4_TGFB3

# repression_TGFB3_by_YREG1: YREG1 repression induction_gene_TGFB3
control_parameter_array[14,1]=1.0 # YREG_TGFB3 #1.0
control_parameter_array[14,2]=1.0 # YREG_TGFB3
# repression_YREG1_by_SNAIL_SLUG: SNAIL_SLUG repression induction_gene_YREG1
control_parameter_array[15,1]=0.1 # GAIN_SNAIL_YREG1 # 0.1
control_parameter_array[15,2]=2.0 # ORDER_SNAIL_YREG1 # 2.0
# activation_LEF1_by_pSmad: pSmad induction induction_gene_LEF1
control_parameter_array[16,1]=0.8# GAIN # _PSMAD_LEF1 # 0.5 # 0.8
control_parameter_array[16,2]=2.0# ORDER# _PSMAD_LEF1       # 1.0, 2.0
# repression_LEF1_by_YREG1: YREG1 repression induction_gene_LEF1
control_parameter_array[17,1]=5.0 # GAIN YREG1_LEF1 # 0.1
control_parameter_array[17,2]=0.5 # ORDER YREG1_LEF1 # 2.0
# activation_vimentin_by_Active_LEF1: Active_LEF1 induction induction_gene_vimentin
control_parameter_array[18,1]=0.3# GAIN_LEF1_VIM
control_parameter_array[18,2]=2.0# ORDER_LEF1_VIM
# activation_SNAIL_SLUG_by_AP1_SP1_P: AP1_SP1_P induction induction_gene_SNAIL_SLUG
control_parameter_array[20,1]=1.0# SS by AP1 SP1
# activation_SNAIL_SLUG_by_ss_virus: ss_virus induction induction_gene_SNAIL_SLUG
control_parameter_array[21,1]=5.0# SS_VIRUS ***

# activation_YAP1_by_ROCK: Active_ROCK induction induction_gene_YAP1
control_parameter_array[22,1] = 0.5;	#	22 Gain: 	activation_YAP1_by_ROCK target: induction_gene_YAP1 actor: Active_ROCK type: induction
...
control_parameter_array[23,1] = 3.2;	#	23 Gain: 	activation_CD1_by_SHEAR_STRESS_2 target: induction_gene_CD31 actor: SHEAR_STRESS_2 type: induction
control_parameter_array[23,2] = 1.5;	#	23 Order: 	activation_CD1_by_SHEAR_STRESS_2 target: induction_gene_CD31 actor: SHEAR_STRESS_2 type: induction
...
addition_start = 105
addition_end = 124
const addition_terms_saved = parameter_array[addition_start:addition_end]
...
# Indices are from the DataFile and are NOT automously updated!!!
 # LEF 1 protein (technically this not not total, due to active amt) #WCB
...
# indices for shear simulations
# 1 "edit_file_none.jl"
# 2 "edit_file_1dyne_per_cm2_0hz.jl",
# 3 "edit_file_5dyne_per_cm2_0hz.jl"
# 4 "edit_file_10dyne_per_cm2_0hz.jl"
# 5 "edit_file_1dyne_per_cm2_1hz.jl"
# 6 "edit_file_5dyne_per_cm2_1hz.jl"
# 7 "edit_file_10dyne_per_cm2_1hz.jl"
...
for sim in simulations
  ...
  sim_ICA = simulations_ICA[i_simdat] # WCB
  data_dictionary_active = deepcopy(xss_data_dict) # RENAME THIS TO RETAIN ORIGINAL call it data_dictionary_active
  (TSIM,X) =  SolveModelGenericPOETS_ram(TSTART,TSTOP,Ts,data_dictionary_active,sim,sim_ICA) # WCB
  i_simdat = i_simdat + 1 # WCB
  push!(sim_data,X)
...
#1 mrna_PECAM (CD31) STEADY SHEAR
MOD  = [sim_data[1][t48,index_cd31_mrna],sim_data[2][t48,index_cd31_mrna],sim_data[3][t48,index_cd31_mrna],
        sim_data[4][t48,index_cd31_mrna]]

QPC_FOLD  = [ST_PECAM[1],ST_PECAM[2],ST_PECAM[3],ST_PECAM[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.05 #nM

obj_array[1]  = SSE(MOD,QPC_REAL)

#2 mrna_asma (ACTA) STEADY SHEAR
MOD  = [sim_data[1][t48,index_asma_vim_mrna],sim_data[2][t48,index_asma_vim_mrna],sim_data[3][t48,index_asma_vim_mrna],
        sim_data[4][t48,index_asma_vim_mrna]]

QPC_FOLD  = [ST_ACTA[1],ST_ACTA[2],ST_ACTA[3],ST_ACTA[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.05 #nM

obj_array[2]  = 0 #SSE(MOD,QPC_FOLD)

#3 mrna_slug STEADY SHEAR
MOD  = [sim_data[1][t48,index_slug_mrna],sim_data[2][t48,index_slug_mrna],sim_data[3][t48,index_slug_mrna],
        sim_data[4][t48,index_slug_mrna]]

QPC_FOLD  = [ST_Slug[1],ST_Slug[2],ST_Slug[3],ST_Slug[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.01

obj_array[3]  = SSE(MOD,QPC_REAL)

#4 mrna_TGFB3 STEADY SHEAR
MOD  = [sim_data[1][t48,index_tgfb3_mrna],sim_data[5][t48,index_tgfb3_mrna],sim_data[6][t48,index_tgfb3_mrna],
        sim_data[7][t48,index_tgfb3_mrna]]

QPC_FOLD  = [ST_TGFB3[1],ST_TGFB3[2],ST_TGFB3[3],ST_TGFB3[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.02

obj_array[4] = SSE(MOD,QPC_REAL)

#5 mrna_PECAM (CD31) OSC
MOD  = [sim_data[1][t48,index_cd31_mrna],sim_data[5][t48,index_cd31_mrna],sim_data[6][t48,index_cd31_mrna],
        sim_data[7][t48,index_cd31_mrna]]

QPC_FOLD  = [OSC_PECAM[1],OSC_PECAM[2],OSC_PECAM[3],OSC_PECAM[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.05

obj_array[5]  = SSE(MOD,QPC_REAL)

#6 mrna_asma (ACTA) OSC
MOD  = [sim_data[1][t48,index_asma_vim_mrna],sim_data[5][t48,index_asma_vim_mrna],sim_data[6][t48,index_asma_vim_mrna],
        sim_data[7][t48,index_asma_vim_mrna]]

QPC_FOLD  = [OSC_ACTA[1],OSC_ACTA[2],OSC_ACTA[3],OSC_ACTA[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.05 #nM
obj_array[6]  = 0#SSE(MOD_FOLD,QPC_FOLD)

#7 mrna_slug OSC
MOD  = [sim_data[1][t48,index_slug_mrna],sim_data[5][t48,index_slug_mrna],sim_data[6][t48,index_slug_mrna],
        sim_data[7][t48,index_slug_mrna]]

QPC_FOLD  = [OSC_Slug[1],OSC_Slug[2],OSC_Slug[3],OSC_Slug[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.01

obj_array[7]  = SSE(MOD,QPC_REAL)

#8 mrna_TGFB3 OSC
MOD  = [sim_data[1][t48,index_tgfb3_mrna],sim_data[5][t48,index_tgfb3_mrna],sim_data[6][t48,index_tgfb3_mrna],
        sim_data[7][t48,index_tgfb3_mrna]]

QPC_FOLD  = [OSC_TGFB3[1],OSC_TGFB3[2],OSC_TGFB3[3],OSC_TGFB3[4]] # FOLD CHANGE
QPC_REAL  = QPC_FOLD*0.02

obj_array[8] = SSE(MOD,QPC_REAL)
...
# apply cross validation hold out (expensive but easy way)
 ...
 #f@show obj_array
 #@show obj_array
 return obj_array+penalty_array
end
...
# See if the system remains stable
  ...
  new_parameter_array[len_rca+12]=5.0# GAIN_ecad_virus
  new_parameter_array[len_rca+21]=5.0# GAIN_SS_virus
  # ORDERS
  new_parameter_array[len_rca+len_cpa_gain+1]=1.0# GAIN_tgfb3_Ab
  new_parameter_array[len_rca+len_cpa_gain+2]=1.0# GAIN_vivt
  new_parameter_array[len_rca+len_cpa_gain+12]=1.0# GAIN_ecad_virus
  new_parameter_array[len_rca+len_cpa_gain+21]=1.0# GAIN_SS_virus
# shear rates are set to 0 in balances and set by initial condition array only
   # machinery
  ...
  # addition terms
 new_parameter_array[addition_start:addition_end] = addition_terms_saved
